package com.cus.jastip.uaa.web.rest;
...
import com.cus.jastip.uaa.repository.UserRepository;
import com.cus.jastip.uaa.security.SecurityUtils;
import com.cus.jastip.uaa.service.MailService;
import com.cus.jastip.uaa.service.UserService;
import com.cus.jastip.uaa.service.dto.UserDTO;
import com.cus.jastip.uaa.web.rest.errors.*;
import com.cus.jastip.uaa.web.rest.vm.KeyAndPasswordVM;
import com.cus.jastip.uaa.web.rest.vm.ManagedUserVM;
...
public class AccountResource {
	...
	@Autowired
	private ProfileSender profileSender;
	...
	public void registerAccount(@Valid @RequestBody ManagedUserVM managedUserVM) {
		...
		// ambil data user ke profile message
		ProfileMessageModel profileMessage = new ProfileMessageModel();
		profileMessage.setLogin(user.getEmail());
		profileMessage.setUserId(user.getId());
		profileMessage.setStatus(ProfileStatus.NEW);
		profileMessage.setFirstname(user.getFirstName());
		profileSender.send(profileMessage);
	}
	...
	public void activateAccount(@RequestParam(value = "key") String key,
		...
		if (user != null) {
			if (user.getActivationKey().equals(key) && user.getActivated() == false) {
				user.setActivated(true);
				userRepository.save(user);
				ProfileMessageModel profileMessage = new ProfileMessageModel();
				profileMessage.setLogin(user.getEmail());
				profileMessage.setUserId(user.getId());
				profileMessage.setStatus(ProfileStatus.ACTIVE);
				profileSender.send(profileMessage);
			}
		}

		/*
		 * Optional<User> user = userService.activateRegistration(key); if
		 * (!user.isPresent()) { throw new
		 * InternalServerErrorException("No user was found for this reset key"); }
		 */
	}
