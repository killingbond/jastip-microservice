...
import com.cus.jastip.transaction.repository.search.CommentSearchRepository;
...
public class CommentResource {
	...
	@GetMapping("/comments/posting/{id}")
	@Timed
	public ResponseEntity<List<Comment>> getCommentsByPosting(@PathVariable Long id, Pageable pageable) {
		log.debug("REST request to get all comments");
		Posting posting = postingRepository.findOne(id);
		List<Comment> list = commentRepository.findByPosting(posting, pageable);
		Page<Comment> page = new PageImpl<>(list);
		HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/comments/posting/" + id);
	 ...
	 *         comment, or with status 400 (Bad Request) if the comment has already
	 *         an ID
	...
	public ResponseEntity<Comment> createComment(@Valid @RequestBody Comment comment) throws URISyntaxException {
		...
		transactionAuditService.addComment(result, ENTITY_NAME, UpdateType.CREATE);
	...
	public ResponseEntity<Comment> updateComment(@Valid @RequestBody Comment comment) throws URISyntaxException {
		...
		Comment result = commentRepository.save(comment);
		commentSearchRepository.save(result);
		transactionAuditService.addComment(result, ENTITY_NAME, UpdateType.UPDATE);
	 ...
	 * @return the ResponseEntity with status 200 (OK) and with body the comment, or
	...
	public ResponseEntity<Void> deleteComment(@PathVariable Long id) {
		...
		transactionAuditService.addComment(result, ENTITY_NAME, UpdateType.DELETE);
	 ...
	 *
	 * @param query
	 *            the query of the comment search
	...
	public ResponseEntity<List<Comment>> searchComments(@RequestParam String query, Pageable pageable) {
		...
		List<Comment> list = StreamSupport
				.stream(commentSearchRepository.search(queryStringQuery(query), pageable).spliterator(), false)
				.collect(Collectors.toList());
		Page<Comment> page = new PageImpl<>(list);
		HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/_search/Comments");
		return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
	}
